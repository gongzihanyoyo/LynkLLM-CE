...
(function (global) {
  ...
  /* MCP 工具返回在界面/存储里保留的最大字符数。
     parts 会整体写进 localStorage，外部工具的返回动辄几十上百 KB，
     原样保存会迅速耗尽配额 → 界面留一份截断副本即可。 */
  const MCP_RESULT_DISPLAY_MAX = 4000;
  ...
  function buildReasoningBlock(text, mode, forceOpen, ms) {
    ...
    const head = el('button', { class: 'reasoning-head', type: 'button' }, [
      /* ⚠️ 这里原来写的是 `bi-brain` —— bootstrap-icons 1.13.1 里**没有**这个图标，
         于是 `::before` 没有 content，思考过程前面一直是个空位（不报错、控制台也不提示）。
         第 13 轮换成确实存在的 bi-lightbulb。改图标名前先确认它存在
         （test14 有全量守卫：源码里每个 bi-* 都要能渲染出字形）。 */
  ...
  /* ======================================================================
     ...
     ----------------------------------------------------------------------
     第 13 轮起分两大类：
       · 内置工具（web_search）—— 结果结构固定，有专门的结果列表样式
       · MCP 工具 —— 结构由外部服务器决定，统一按「调用参数 + 原始返回」展示
     分派靠 tool.kind 字段（'mcp' / 缺省即内置），而不是靠名字猜。
     ====================================================================== */

  function isMcpTool(tool) {
    return !!(tool && (tool.kind === 'mcp' || tool.serverId));
  }

  function toolIcon(tool) {
    if (isMcpTool(tool)) {
      if (tool.status === 'running') return 'bi-plug';
      if (tool.status === 'error') return 'bi-plug';
      return 'bi-plug-fill';
    }
  ...
  /**
   * 工具块的标题文案。
   * MCP 工具**必须把工具名外显**出来（用户明确要求），格式：服务器名 · 工具名。
   * 连工具名都拿不到时才退回泛称。
   */
  function toolLabel(tool) {
    if (isMcpTool(tool)) {
      const t = tool.toolName || '';
      const s = tool.serverName || '';
      if (s && t) return s + ' · ' + t;
      return t || s || T('toolMcp');
    }
  ...
  function toolMetaText(tool) {
    if (tool.status === 'running') return isMcpTool(tool) ? T('toolMcpCalling') : T('toolSearching');
    if (tool.status === 'error') return isMcpTool(tool) ? T('toolMcpFailed') : T('toolSearchFailed');
    if (isMcpTool(tool)) {
      const bits = [T('toolMcpDone')];
  ...
  /** 工具块的副标题：MCP 工具把调用参数摘要放在标题旁边，一眼能看出「拿什么参数调的」 */
  function toolSubtitle(tool) {
    if (!isMcpTool(tool)) {
      return tool.query ? ('“' + truncate(tool.query, 48) + '”') : '';
    }
  ...
  /** 保留换行的截断（用于工具返回的原文，别把 JSON 的排版压掉） */
  function truncateKeepLines(s, n) {
    const t = String(s == null ? '' : s);
    return t.length > n ? (t.slice(0, n) + '\n…[' + T('toolTruncated') + ']') : t;
  }

  /** MCP 工具块的展开内容：调用参数 + 返回结果（用户要求「展开后显示调用过程和返回结果」） */
  function fillMcpToolBody(body, tool) {
    clear(body);

    if (tool.status === 'running') {
      body.appendChild(el('div', { class: 'tool-status' }, [
        el('span', { class: 'thinking-dots' }, [el('span'), el('span'), el('span')]),
        el('span', { text: T('toolMcpCalling') })
      ]));
    }

    if (tool.status === 'error') {
      body.appendChild(el('div', { class: 'tool-error', text: tool.error || T('toolMcpFailed') }));
    }

    // 调用信息：服务器 / 工具 / 参数
    const info = el('div', { class: 'tool-mcp-info' });
    const rows = [
      ['server', tool.serverName || ''],
      ['tool', tool.toolName || tool.name || '']
    ];
    rows.forEach(([k, v]) => {
      if (!v) return;
      info.appendChild(el('div', { class: 'tool-mcp-row' }, [
        el('span', { class: 'tool-mcp-key', text: T('toolMcpField_' + k) }),
        el('span', { class: 'tool-mcp-val', text: v })
      ]));
    });
    if (info.children.length) body.appendChild(info);

    const args = tool.args || {};
    const argKeys = Object.keys(args).filter(k => k !== '__raw');
    if (argKeys.length) {
      const det = el('details', { class: 'tool-params', open: true });
      det.appendChild(el('summary', { text: T('toolMcpArgs') }));
      det.appendChild(el('pre', { text: JSON.stringify(args, null, 2) }));
      body.appendChild(det);
    }

    if (tool.resultText) {
      const det = el('details', { class: 'tool-params tool-mcp-result', open: true });
      det.appendChild(el('summary', { text: T('toolMcpResult') }));
      det.appendChild(el('pre', { text: tool.resultText }));
      body.appendChild(det);
    } else if (tool.status === 'done' && !tool.isError) {
      body.appendChild(el('div', { class: 'tool-empty', text: T('toolNoResult') }));
    }

    if (tool.isError) {
      body.appendChild(el('div', { class: 'tool-error', text: T('toolMcpIsError') }));
    }
  }

  function fillToolBody(body, tool) {
    if (isMcpTool(tool)) return fillMcpToolBody(body, tool);
  ...
  /**
   * 构建工具调用折叠块
   * @param {object} tool { id, name, kind, serverId, serverName, toolName, query, args,
   *                        status, results, answer, resultText, isError, error, ms, images }
   * @param {boolean} [live] 是否仍在进行中（默认展开）
   */
  function buildToolBlock(tool, live) {
    const running = tool.status === 'running';
    const open = live != null ? !!live : running;
    const cls = 'tool-call ' + (running ? 'live' : (tool.status === 'error' ? 'err' : 'done'))
      + (isMcpTool(tool) ? ' mcp' : '') + (open ? ' open' : '');
    ...
    const head = el('button', { class: 'tool-head', type: 'button' }, [
      ...
      el('span', { class: 'tool-query', text: toolSubtitle(tool) }),
  ...
  function updateToolBlock(box, tool) {
    ...
    const label = box.querySelector('.tool-label');
    if (label) label.textContent = toolLabel(tool);
    const q = box.querySelector('.tool-query');
    if (q) q.textContent = toolSubtitle(tool);
  ...
  /** 当前对话的联网搜索开关：on 开启（由模型自行决定是否搜索） | off 关闭 */
  function currentWebSearch() {
    const conv = getConv();
    const v = (conv && conv.webSearch) || settings().webSearch || 'on';
    return v === 'off' ? 'off' : 'on';
  }

  /** 设置开关。mode 传 'on' | 'off'（旧值 'auto' 归一为 'on'） */
  function setWebSearch(mode) {
    const v = mode === 'off' ? 'off' : 'on';
    const conv = getConv();
  ...
  /* ======================================================================
     MCP 开关（与联网搜索同级，但有多台服务器）
     ====================================================================== */

  /**
   * 某台 MCP 服务器在当前对话里是否启用。
   * 取值顺序：对话覆盖 → 全局默认（settings.mcpDisabled）→ 默认启用。
   * 「默认启用」是刻意的：用户添加一台服务器就是为了用它。
   */
  function mcpServerEnabled(serverId) {
    const conv = getConv();
    const overrides = (conv && conv.mcp && typeof conv.mcp === 'object') ? conv.mcp : {};
    if (Object.prototype.hasOwnProperty.call(overrides, serverId)) return overrides[serverId] === true;
    return (Store.getSettings().mcpDisabled || []).indexOf(serverId) < 0;
  }

  function setMcpServerEnabled(serverId, on) {
    const conv = getConv();
    if (conv) {
      if (!conv.mcp || typeof conv.mcp !== 'object') conv.mcp = {};
      conv.mcp[serverId] = !!on;
      touch(conv);
    } else {
      const off = (Store.getSettings().mcpDisabled || []).filter(x => x !== serverId);
      if (!on) off.push(serverId);
      Store.saveSettings({ mcpDisabled: off });
    }
    // 开关状态变了 → 工具清单要重算
    if (global.Mcp) Mcp.forget(serverId);
    if (global.App && App.onModelChanged) App.onModelChanged();
  }

  /** 当前对话实际启用的 MCP 服务器列表 */
  function enabledMcpServers() {
    return Store.getMcpServers().filter(s => s.url && mcpServerEnabled(s.id));
  }

  function mcpToolCount() {
    return Store.getMcpServers().length;
  }

  /* ======================================================================
     输入框左侧「+」菜单
     ----------------------------------------------------------------------
     第 13 轮起，这里成为「加东西进这条消息」的统一入口：
       · 上传图片（原来是一个独立的 chip 按钮，现在收进来了）
       · ——分割线——
       · Tavily 开关（原来的「联网搜索 · 自动」chip 已移除，改成开关形态）
       · MCP 子菜单（每台服务器一个开关，预留继续追加其它工具的位置）
     菜单项刻意不带描述文字（用户明确要求），所以用 popover-item 的紧凑形态。
     ====================================================================== */

  /**
   * 一行「名字 + 开关」的菜单项（点击整行即切换，开关只作视觉指示）。
   *
   * ⚠️ 这里的开关**刻意不复用 `.switch-track` / `.switch-thumb`**：
   * 那两个类名在「控件玻璃底」那条批量规则的白名单里
   * （`html[data-glass="on"] :is(.switch-track):not(…)`，权重 0,4,1），
   * 会把轨道刷成近白半透明 —— 于是**开关永远是「关」的样子**，
   * 即使 `.menu-switch.on` 确实生效（滑块移动了、但轨道颜色被盖掉）。
   * 换一组专属类名（.ms-track / .ms-thumb）就彻底避开这场权重战，
   * 与「不要去和批量规则的权重硬碰，绕开它」的既有结论一致。
   */
  function menuSwitchRow(opts) {
    const on = !!opts.on;
    const row = el('button', {
      class: 'menu-switch' + (on ? ' on' : '') + (opts.disabled ? ' disabled' : ''),
      type: 'button',
      role: 'menuitemcheckbox',
      'aria-checked': on ? 'true' : 'false'
    }, [
      el('i', { class: 'bi ' + (opts.icon || 'bi-circle') }),
      el('span', { class: 'ms-label', text: opts.label }),
      el('span', { class: 'ms-indicator', 'aria-hidden': 'true' }, [
        el('span', { class: 'ms-track' }, [el('span', { class: 'ms-thumb' })])
      ])
    ]);
    if (opts.disabled) row.setAttribute('disabled', 'disabled');
    else row.addEventListener('click', e => {
      e.stopPropagation();
      opts.onToggle(!on);
    });
    return row;
  ...
  /** 子菜单里的一行普通入口（用于「MCP 设置」这类跳转） */
  function menuActionRow(opts) {
    const row = el('button', { class: 'popover-item compact', type: 'button' }, [
      el('i', { class: 'bi ' + (opts.icon || 'bi-circle') }),
      el('div', { class: 'pi-body' }, [el('div', { class: 'pi-title', text: opts.label })])
    ]);
  ...
  /**
   * 构建「+」菜单内容并打开。
   * @param {HTMLElement} anchorEl 「+」按钮
   */
  function openPlusMenu(anchorEl) {
    const box = el('div');

    /* --- 1. 上传图片（受当前模型是否支持图片输入影响） --- */
    const model = currentModel();
    const canAttach = !model || model.supportsImages !== false;
    const imgRow = el('button', {
      class: 'popover-item compact' + (canAttach ? '' : ' disabled'),
      type: 'button'
    }, [
      el('i', { class: 'bi bi-image' }),
      el('div', { class: 'pi-body' }, [el('div', { class: 'pi-title', text: T('uploadImage') })])
    ]);
    if (canAttach) {
      ...
      imgRow.setAttribute('disabled', 'disabled');
    }
    ...
    /* --- 2. Tavily 开关（开 / 关） --- */
    const tavilyOn = tavilyReady() && currentWebSearch() !== 'off';
    box.appendChild(menuSwitchRow({
      icon: 'bi-globe2',
      label: T('webSearch'),
      on: tavilyOn,
      ...
      onToggle: v => {
        ...
        setWebSearch(v ? 'on' : 'off');
        Popover.close();
      }
    }));
    if (!tavilyReady()) {
      box.appendChild(menuActionRow({
        icon: 'bi-key',
        label: T('webSearchNeedTavily'),
        onClick: () => { if (global.Settings) Settings.open('tavily'); }
      }));
    }

    /* --- 3. MCP 子菜单 --- */
    box.appendChild(buildMcpSubmenu());

    Popover.open(anchorEl, box, { align: 'start', up: true, width: 268 });
  }

  /**
   * MCP 子菜单 —— 点击后在**主菜单旁边**另起一个浮层（第 14 轮改）。
   *
   * 之前是「在主菜单内部向下展开」，缺点是菜单会突然变高、把下面的输入区顶开，
   * 而且子项一多就把主菜单挤得很长。改成二级浮层后两者互不干扰。
   *
   * 结构刻意做成「一段可增长的列表」：以后要加别的工具类（本地脚本、知识库…），
   * 照着建一个新的 buildXxxSubmenu() 追加即可，不必动上面的骨架。
   */
  function buildMcpSubmenu() {
    const servers = Store.getMcpServers();
    const enabledCount = servers.filter(s => mcpServerEnabled(s.id)).length;

    const head = el('button', {
      class: 'popover-item compact menu-sub-head',
      type: 'button',
      'aria-haspopup': 'dialog',
      'aria-expanded': 'false'
    }, [
      el('i', { class: 'bi bi-puzzle' }),
      el('div', { class: 'pi-body' }, [el('div', { class: 'pi-title', text: 'MCP' })]),
      el('span', { class: 'menu-badge', text: String(enabledCount) }),
      el('i', { class: 'bi bi-chevron-right menu-sub-caret' })
    ]);

    head.addEventListener('click', e => {
      e.stopPropagation();
      if (Popover.isSubOpen()) {
        Popover.closeSub();
        head.setAttribute('aria-expanded', 'false');
        return;
      }
      const node = Popover.openSub(head, buildMcpSubContent(head), { width: 258 });
      if (node) head.setAttribute('aria-expanded', 'true');
    });

    return head;
  }

  /** 二级浮层的内容（与主菜单同一套行组件，观感一致） */
  function buildMcpSubContent(head) {
    const servers = Store.getMcpServers();
    const box = el('div');

    box.appendChild(el('div', { class: 'popover-label', text: 'MCP' }));

    if (!servers.length) {
      box.appendChild(el('div', { class: 'menu-empty', text: T('mcpNone') }));
    } else {
      servers.forEach(s => {
        box.appendChild(menuSwitchRow({
          icon: 'bi-plug',
          label: s.name || s.url,
          on: mcpServerEnabled(s.id),
          onToggle: v => {
            setMcpServerEnabled(s.id, v);
            Popover.close();          // 关主浮层会连二级浮层一起收掉
          }
        }));
      });
    }
    ...
    box.appendChild(menuActionRow({
      icon: 'bi-plus-lg',
      label: T('mcpManage'),
      onClick: () => { if (global.Settings) Settings.open('mcp'); }
    }));

    return box;
  }

  /* 兼容旧调用点：过去聊天区有独立的「联网搜索」chip（第 13 轮已移除）。
     保留这个函数名，方便老代码/老测试继续可用。 */
  function openWebSearchPicker(anchorEl) {
    openPlusMenu(anchorEl);
  }
  ...
  function runCompletion(conv, model, opts) {
    ...
    /* 工具清单（本轮）：
       · 内置联网搜索（Tavily）—— 仅在模型声明支持工具调用、已配 Key 且开关开启时加入
       · MCP 外部工具 —— 同样要求模型支持工具调用；工具清单在下面异步取（见 prepareTools） */
    const searchEnabled = webSearchActive(model);
    const toolsSupported = !!(model && model.supportsTools);
    const mcpServers = toolsSupported ? enabledMcpServers() : [];
    let tools = searchEnabled ? [API.tavilyTool(Store.getTavilyForced())] : null;
    let mcpByName = {};      // 模型可见的工具名 -> { server, toolName }
    let mcpErrors = [];      // 连不上的服务器（界面提示用，不影响对话）
    ...
    async function execTools(calls, preContent) {
      ...
      for (const c of calls) {
        ...
        /* 分派：MCP 工具的名字形如 mcp__<serverSlug>__<toolName>，
           先用 mcpByName 精确查（本轮实际下发给模型的那张表），
           再退回按名字前缀反解（模型自己编了个名字 / 上一轮的轨迹回放时用）。 */
        const hit = mcpByName[c.function.name]
          || (global.Mcp ? Mcp.parseExternalName(c.function.name, mcpServers) : null);
        const isMcp = !!hit;
        const tool = {
          type: 'tool',
          id: c.id,
          ...
          serverId: isMcp ? (hit.server.id || '') : '',
          serverName: isMcp ? (hit.server.name || '') : '',
          toolName: isMcp ? hit.toolName : '',
        ...
        try {
          if (isMcp) {
            const r = await Mcp.callTool(hit.server, hit.toolName, args || {}, { signal: controller.signal });
            tool.ms = r.ms || (Date.now() - t0);
            /* ⚠️ 只留一份「给人看」的截断副本：parts 会整体进 localStorage，
               MCP 的返回动辄几十上百 KB，原样存下来会迅速撑爆存储配额。
               模型拿到的仍是 formatMcpForModel 里那份（更长、且是临时的）。 */
            tool.resultText = truncateKeepLines(r.text, MCP_RESULT_DISPLAY_MAX);
            tool.isError = r.isError === true;
            // 服务器说失败了 → 块标成错误态（但**不抛异常**，仍要把结果回给模型让它解释）
            tool.status = tool.isError ? 'error' : 'done';
            if (tool.isError) tool.error = truncate(r.text, 300) || T('toolMcpFailed');
            payload = API.formatMcpForModel(r);
          } else {
          ...
          tool.resultText = tool.resultText || tool.error;
          updateToolBlock(node, tool);
          payload = isMcp
            ? JSON.stringify({
              error: tool.error,
              note: 'The MCP tool call failed. Tell the user what went wrong; '
    ...
    /**
     * 组装本轮要交给模型的工具清单。
     * 内置工具（Tavily）同步就能定；MCP 工具需要先向每台服务器 `tools/list`，
     * 所以这里要等一会儿。任何一台连不上都不阻断对话 —— 只记进 mcpErrors。
     */
    async function prepareTools() {
      if (!mcpServers.length || !global.API || !API.buildMcpTools) return;
      let r;
      try {
        r = await API.buildMcpTools(mcpServers, { signal: controller.signal });
      } catch (e) {
        mcpErrors = [{ serverId: '', serverName: '', error: (e && e.message) || String(e) }];
        return;
      }
      mcpErrors = r.errors || [];
      mcpByName = r.byName || {};
      ...
      // 有服务器连不上：如实告知，否则用户会以为「工具已经生效了」
      if (mcpErrors.length && global.UI && UI.Toast) {
        /* ⚠️ 通知是**附属行为**：它自己出错绝不能把整轮对话带崩。
           （真踩过：这里写成了不存在的 UI.Toast.warn，异常一路冒泡到
           runCompletion 的 catch，整个请求被判成失败 —— 用户看到的是
           「请求失败」，而真正的原因只是少提示了一句话。） */
        try {
          const names = mcpErrors.map(e => e.serverName || e.serverId || 'MCP').join('、');
          UI.Toast.warning(T('mcpToolsUnavailable', names));
        } catch (e) { /* 提示失败不影响对话 */ }
      }
    }
    ...
    /* ⚠️ 先取 MCP 工具清单再进对话循环。取清单是网络操作，失败已内部吞掉，
       绝不会因为「MCP 连不上」就让整轮对话报错（那是最糟的体验）。 */
    return prepareTools().then(runLoop).then(() => {
  ...
  Chat.openPlusMenu = openPlusMenu;
  Chat.mcpServerEnabled = mcpServerEnabled;
  Chat.setMcpServerEnabled = setMcpServerEnabled;
  Chat.enabledMcpServers = enabledMcpServers;
