...
(function (global) {
  ...
  /**
   * 展示这条回复由哪个模型生成。
   * 优先用消息自带的 modelName（新消息都会写入）；历史消息没有该字段时
   * 用 ctx.modelFallback 兜底，避免老数据一片空白。
   * @param {object} msg 消息
   * @param {object} ctx 操作上下文（来自 actionCtx()；modelFallback 由它成批算一次，
   *   自己手搓 ctx 时要一并带上，否则老消息不显示模型名）
   */
  function buildModelRow(msg, ctx) {
    if (!msg || msg.role !== 'assistant') return null;
    const name = msg.modelName || (ctx && ctx.modelFallback) || '';
    if (!name) return null;

    const row = el('div', { class: 'msg-model' }, [
      el('i', { class: 'bi bi-cpu', 'aria-hidden': 'true' }),
      el('span', { class: 'msg-model-name', text: name })
    ]);
    const detail = [msg.model, msg.via && msg.via !== 'chat' ? msg.via : '']
      .filter(Boolean).join(' · ');
    row.title = T('generatedBy', name) + (detail ? '  ·  ' + detail : '');
    return row;
  }

  /**
   * 组装消息底部区域：先 Token 用量行，再操作按钮行，最后生成模型行
   * @param {object} msg 消息
   * @param {object} ctx 必须来自 actionCtx()（它负责带上老消息的兜底模型名）
  ...
  function buildFooter(msg, ctx, isLatest) {
    ...
    const modelRow = buildModelRow(msg, ctx);
    if (modelRow) frag.appendChild(modelRow);
    return frag;
  }
  ...
  function syncActionBars(conv) {
    ...
    const ctx = actionCtx(convFallbackModelName());
