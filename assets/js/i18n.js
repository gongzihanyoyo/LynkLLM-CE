...
(function (global) {
  ...
  const DICT = {
    'zh-CN': {
      ...
      /* ---- 第 13 轮：输入框「+」菜单 / 快捷键 / MCP ---- */
      tabShortcuts: '快捷键',
      plusMenu: '添加',
      uploadImage: '上传图片',

      /* 快捷键页 */
      shortcutsMacNote: '在 macOS 上，Ctrl 对应 ⌘（Command）。',
      scColKeys: '快捷键',
      scColAction: '功能',
      scGroupChat: '对话',
      scGroupView: '界面',
      scNewChat: '新建对话',
      scSearchChats: '搜索历史对话',
      scExportChat: '导出当前对话',
      scToggleSidebar: '切换侧边栏',
      scOpenSettings: '打开设置',
      scEscape: '关闭浮层 / 停止生成',

      /* Tavily 开关（语义从「关闭/自动」改为「开/关」） */
      webSearchOn: '开启',
      webSearchSwitchedOn: '联网搜索已开启',
      webSearchSwitchedOff: '联网搜索已关闭',
      exportNoConv: '当前没有可导出的对话',
      ...
      toolMcpDone: '调用完成',
      toolMcpFailed: '调用失败',
      toolMcpIsError: '工具返回了错误结果',
      toolMcpArgs: '调用参数',
      toolMcpResult: '返回结果',
      toolMcpField_server: '服务器',
      toolMcpField_tool: '工具',
      toolTruncated: '内容过长，已截断',
      mcpHint: '将外部工具与 AI 集成，需要模型支持 Tool Call。',
      mcpDesc: '按 Streamable HTTP 协议连接外部工具服务器。配置仅保存在本机浏览器。',
      mcpNotice: '目前仅支持 Streamable HTTP 协议；暂不支持 OAuth 授权。',
      addMcp: '添加服务器',
      editMcp: '编辑服务器',
      mcpUrl: '连接地址',
      mcpUrlHint: 'Streamable HTTP 端点，通常以 /mcp 结尾',
      mcpStream: '支持流式输出',
      mcpStreamHint: '开启后以流式方式接收响应，工具执行过程可边跑边显示',
      mcpHeaders: '自定义 HTTP 头（可选）',
      mcpHeadersHint: '用于鉴权等场景，例如 Authorization: Bearer xxx。Content-Type / Accept 等协议头由程序自动管理，填了会被忽略。',
      mcpHeaderCount: '{0} 个自定义头',
      mcpEmpty: '还没有配置 MCP 服务器',
      mcpEmptySub: '添加一个 Streamable HTTP 端点，把外部工具接进来',
      mcpNone: '尚未配置服务器',
      mcpManage: '管理 MCP 服务器…',
      mcpEnabled: '已启用',
      mcpDisabled: '已停用',
      mcpEnable: '启用',
      mcpDisable: '停用',
      mcpToolCount: '{0} 个工具',
      mcpUnverified: '未连通',
      mcpToolsUnavailable: '以下 MCP 服务器连不上，本轮未启用其工具：{0}',
      /* 混合内容（第 14 轮）：https 页面请求 http 地址会被浏览器直接拦死 */
      mcpMixedHint: '⚠️ 当前页面是 HTTPS，而该地址是 HTTP：浏览器会拦截这类请求（混合内容），必须连接增强脚本后由它转发',
      mcpMixedHintOk: '该地址是 HTTP：请求会经增强脚本转发（已连接），不受混合内容限制',
      mcpMixedNeedScript: '页面是 HTTPS、地址是 HTTP —— 浏览器会拦截（混合内容）。请安装并连接增强脚本，或改用 HTTPS 地址',
      mcpMixedViaScript: 'HTTP 地址将经增强脚本转发（混合内容不受限制）',
      mcpErrMixedContent: '当前页面是 HTTPS，而该地址是 HTTP，浏览器会拦截这类请求（混合内容）。请连接增强脚本后由它代为转发，或把 MCP 服务改为 HTTPS。',
      mcpTesting: '正在连接…',
      mcpTestOk: '连接成功 · {0} 个工具 · {1}ms',
      mcpTestFail: '连接失败',
      mcpNeedName: '请填写显示名称',
      mcpNeedUrl: '请填写连接地址',
      mcpUrlScheme: '连接地址需以 http:// 或 https:// 开头',
      add: '添加',
      deleteMcpTitle: '删除 MCP 服务器',
      deleteMcpText: '确定删除「{0}」吗？该操作不可撤销。',
      statMcp: 'MCP 服务器'
    },
    ...
    en: {
      ...
      fCorsBypassHint: 'Available once the enhancer script is connected',
      ...
      /* ---- Round 13: plus menu / shortcuts / MCP ---- */
      tabShortcuts: 'Shortcuts',
      plusMenu: 'Add',
      uploadImage: 'Upload image',

      shortcutsMacNote: 'On macOS, Ctrl means ⌘ (Command).',
      scColKeys: 'Shortcut',
      scColAction: 'Action',
      scGroupChat: 'Chat',
      scGroupView: 'Interface',
      scNewChat: 'New chat',
      scSearchChats: 'Search conversations',
      scExportChat: 'Export current chat',
      scToggleSidebar: 'Toggle sidebar',
      scOpenSettings: 'Open settings',
      scEscape: 'Close overlay / stop generating',

      webSearchOn: 'On',
      webSearchSwitchedOn: 'Web search enabled',
      webSearchSwitchedOff: 'Web search disabled',
      exportNoConv: 'There is no conversation to export',

      toolMcp: 'MCP',
      toolMcpCalling: 'Calling…',
      toolMcpDone: 'Done',
      toolMcpFailed: 'Call failed',
      toolMcpIsError: 'The tool returned an error',
      toolMcpArgs: 'Arguments',
      toolMcpResult: 'Result',
      toolMcpField_server: 'Server',
      toolMcpField_tool: 'Tool',
      toolTruncated: 'truncated',
      mcpHint: 'Integrate external tools with the AI. The model must support Tool Call.',
      mcpDesc: 'Connect external tool servers over Streamable HTTP. Configuration stays in this browser.',
      mcpNotice: 'Only the Streamable HTTP transport is supported; OAuth is not supported yet.',
      addMcp: 'Add server',
      editMcp: 'Edit server',
      mcpUrl: 'Endpoint URL',
      mcpUrlHint: 'Streamable HTTP endpoint, usually ending in /mcp',
      mcpStream: 'Streaming responses',
      mcpStreamHint: 'Consume the response as a stream so tool progress can be shown while it runs',
      mcpHeaders: 'Custom HTTP headers (optional)',
      mcpHeadersHint: 'Useful for auth, e.g. Authorization: Bearer xxx. Protocol headers such as '
        + 'Content-Type / Accept are managed automatically and will be ignored if you set them.',
      mcpHeaderCount: '{0} custom header(s)',
      mcpEmpty: 'No MCP server configured yet',
      mcpEmptySub: 'Add a Streamable HTTP endpoint to bring in external tools',
      mcpNone: 'No server configured',
      mcpManage: 'Manage MCP servers…',
      mcpEnabled: 'Enabled',
      mcpDisabled: 'Disabled',
      mcpEnable: 'Enable',
      mcpDisable: 'Disable',
      mcpToolCount: '{0} tool(s)',
      mcpUnverified: 'Unreachable',
      mcpToolsUnavailable: 'These MCP servers are unreachable, so their tools were skipped: {0}',
      mcpMixedHint: '⚠️ This page is HTTPS but the endpoint is HTTP — the browser blocks such '
        + 'requests (mixed content). The enhancer script must be connected to forward them.',
      mcpMixedHintOk: 'The endpoint is HTTP; requests go through the enhancer script (connected), '
        + 'so mixed-content blocking does not apply.',
      mcpMixedNeedScript: 'Page is HTTPS but the endpoint is HTTP — the browser blocks this '
        + '(mixed content). Please install and connect the enhancer script, or use an HTTPS URL.',
      mcpMixedViaScript: 'The HTTP endpoint will be reached through the enhancer script '
        + '(mixed content does not apply).',
      mcpErrMixedContent: 'This page is HTTPS while the endpoint is HTTP, so the browser blocks '
        + 'the request (mixed content). Connect the enhancer script to forward it, or switch the '
        + 'MCP service to HTTPS.',
      mcpTesting: 'Connecting…',
      mcpTestOk: 'Connected · {0} tool(s) · {1}ms',
      mcpTestFail: 'Connection failed',
      mcpNeedName: 'Please enter a display name',
      mcpNeedUrl: 'Please enter the endpoint URL',
      mcpUrlScheme: 'The URL must start with http:// or https://',
      add: 'Add',
      deleteMcpTitle: 'Delete MCP server',
      deleteMcpText: 'Delete “{0}”? This cannot be undone.',
      statMcp: 'MCP servers'
    }
  };
