...
(function (global) {
   ...
   * @param {object} [opts] { silent } silent=true 时「复制成功/失败」都不提示 ——
   *   用于「顺手帮用户复制」这类附属行为，失败不该打断主流程、更不该报错吓人。
   */
  function copyText(text, opts) {
    opts = opts || {};
    const silent = opts.silent === true;
    const done = () => { if (!silent) Toast.success(I18N.t('copiedToClipboard')); };
    const fail = () => {
      ...
      try {
        ...
        if (ok) done(); else if (!silent) Toast.error(I18N.t('copyFailed'));
      } catch (e) {
        if (!silent) Toast.error(I18N.t('copyFailed'));
      }
    };
  ...
  const Popover = (function () {
    ...
    /* 二级浮层（「+」菜单里的 MCP 子菜单用它）：与主浮层同时存在、
       挂在主浮层**旁边**，主浮层关闭时一起关掉。 */
    let sub = null, subAnchor = null;
    const GAP = 8;
    const SUB_GAP = 4;
    ...
    function close(opts) {
      if (!current) return;
      opts = opts || {};
      closeSub({ instant: true });      // 二级浮层跟着主浮层一起收
    ...
    function onLayerDown(e) {
      // 点在二级浮层、或它的触发行上 → 不关（否则二级浮层刚打开就被关掉）
      if (sub && (sub.contains(e.target) || (subAnchor && subAnchor.contains(e.target)))) return;
    ...
    function open(anchorEl, content, opts) {
      ...
      /* ⚠️ .popover-layer 的 z-index 是 150，低于 .modal-backdrop(200) /
         堆叠弹窗(260) —— 从弹窗内部打开的浮层会被弹窗整个盖住（看不见）。
         需要盖过弹窗时传 layerZ（自建下拉框用在设置页里就是这种情况）。
         不传就显式清空，避免上一次抬高的值残留影响后续浮层。 */
      L.style.zIndex = opts.layerZ ? String(opts.layerZ) : '';
    ...
    /**
     * 二级浮层的定位：贴在主浮层**旁边**。
     * 优先放右边（子菜单在右侧才符合「展开」的直觉），右边放不下就翻到左边；
     * 竖直方向与触发它的那一行对齐（顶到边就往下收）。
     */
    function positionSub(node, anchorEl) {
      const r = anchorEl.getBoundingClientRect();
      const vw = doc.documentElement.clientWidth;
      const vh = doc.documentElement.clientHeight;
      ...
      const mainRect = current ? current.getBoundingClientRect() : null;
      let left;
      if (mainRect && mainRect.right + SUB_GAP + pw <= vw - 8) {
        left = mainRect.right + SUB_GAP;              // 主浮层右侧
      } else if (mainRect) {
        left = mainRect.left - SUB_GAP - pw;          // 右侧不够 → 左侧
      } else {
        left = r.right + SUB_GAP;
      }
      left = Math.max(8, Math.min(left, vw - pw - 8));
    ...
    /**
     * 打开一个**二级浮层**（主浮层保持开着）。
     * 用途：菜单里的「子菜单」——在菜单旁边另起一个浮层，
     * 而不是在原菜单内部撑开（用户第 14 轮明确要求）。
     * @param {HTMLElement} anchorEl 触发它的那一行
     * @param {HTMLElement} content
     * @param {object} [opts] { width, className, layerZ }
     * @returns {HTMLElement|null}
     */
    function openSub(anchorEl, content, opts) {
      opts = opts || {};
      if (!current) return null;          // 没有主浮层时不该有二级浮层
      closeSub({ instant: true });
      const L = ensure();
      if (!L) return null;

      const node = el('div', {
        class: 'popover popover-sub' + (opts.className ? ' ' + opts.className : '')
      }, [content]);
      ...
      positionSub(node, anchorEl || current);
      return node;
    }

    function closeSub(opts) {
      if (!sub) return;
      opts = opts || {};
      const node = sub;
      sub = null;
      ...
      const teardown = () => { if (node && node.parentNode) node.parentNode.removeChild(node); };
      const reduce = global.matchMedia && global.matchMedia('(prefers-reduced-motion: reduce)').matches;
      if (reduce || opts.instant) teardown();
      else {
        node.classList.add('closing');
        setTimeout(teardown, 160);
      }
    ...
    /** 二级浮层当前开着吗（供测试与状态判断） */
    function isSubOpen() { return !!sub; }

    return {
      open,
      close,
      isOpen: () => !!current,
      /* 二级浮层：菜单里的「子菜单」在菜单**旁边**另起一个浮层（第 14 轮） */
      openSub,
      closeSub,
      isSubOpen,
      /** 当前打开的子浮层节点（测试用） */
      ...
      reposition: (o) => {
        ...
        position(current, anchor, o || {});
        if (sub && subAnchor) positionSub(sub, subAnchor);
      }
    };
  })();
  ...
  const Theme = (function () {
    ...
    function apply(setting) {
      ...
      // 主题色需要按深浅主题重新推导明暗，因此切主题后重算一次个性化变量
      if (global.Personalize && global.Personalize.apply) global.Personalize.apply();
  ...
  /* ==========================================================================
     自建下拉选择框（UI.Select）
     --------------------------------------------------------------------------
     为什么要有它：原生 <select> 的**展开列表由浏览器/系统绘制**，无法适配
     毛玻璃主题 —— 在玻璃或深色面板上会闪出一个系统风格的白列表，很突兀。
     这里在原生 select 之上盖一个自建按钮 + 浮层列表。

     ⚠️ 刻意保留原生 select 在 DOM 里（视觉隐藏、不可交互）作为
     「值 / 选项 / change 事件」的唯一数据源：
       · 既有代码 `sel.value = x` 照常生效；
       · 既有代码 `sel.dispatchEvent(new Event('change'))` 照常生效；
       · 测试里 `querySelectorAll('#x option')` 照常能查到；
     这样换 UI 不牵动业务逻辑与既有测试契约，风险最小。
     代价：代码里直接改 .value 而不派发 change 时按钮文字会滞后 ——
     所以提供 refreshAll()，在设置页打开 / 切页后调用一次即可。
     ========================================================================== */
  const Select = (function () {
    ...
    /** 把某个 select 的当前值同步到自建按钮的显示文字上 */
    function refresh(sel) {
      if (!sel || !sel.parentNode) return;
      const trig = sel.parentNode.querySelector('.ui-select-trigger');
      if (!trig) return;
      const label = trig.querySelector('.ui-select-label');
      if (label) label.textContent = labelOf(sel);
      trig.disabled = !!sel.disabled;
      trig.setAttribute('aria-disabled', sel.disabled ? 'true' : 'false');
    }

    function refreshAll(root) {
      const scope = root || doc;
      if (!scope.querySelectorAll) return;
      Array.prototype.forEach.call(scope.querySelectorAll(SEL + '[data-ui-select]'), refresh);
    }
    ...
    function openMenu(sel, trig) {
      const menu = el('div', { class: 'ui-select-menu', role: 'listbox' });
      Array.prototype.forEach.call(sel.options, (o, i) => {
        const on = i === sel.selectedIndex;
        const item = el('button', {
          class: 'ui-select-option' + (on ? ' is-selected' : ''),
          type: 'button', role: 'option', 'aria-selected': on ? 'true' : 'false'
        }, [
          el('span', { class: 'ui-select-option-text', text: String(o.textContent || '').trim() || o.value }),
          el('i', { class: 'bi bi-check2' })
        ]);
        item.addEventListener('click', () => {
          pick(sel, i);
          Popover.close();
          try { trig.focus(); } catch (e) { /* noop */ }
        });
        menu.appendChild(item);
      });

      Popover.open(trig, menu, {
        align: 'start',
        width: Math.max(200, Math.round(trig.getBoundingClientRect().width)),
    ...
    /** 把 root 下所有原生 select 升级为自建下拉（幂等，可重复调用） */
    function enhance(root) {
      const scope = root || doc;
      if (!scope.querySelectorAll) return;
      Array.prototype.forEach.call(scope.querySelectorAll(SEL + ':not([data-ui-select])'), sel => {
        sel.setAttribute('data-ui-select', '1');
        sel.classList.add('ui-select-native');
        sel.setAttribute('tabindex', '-1');
        sel.setAttribute('aria-hidden', 'true');

        const trig = el('button', {
          class: 'select ui-select-trigger', type: 'button',
          role: 'combobox', 'aria-haspopup': 'listbox', 'aria-expanded': 'false'
        }, [
          el('span', { class: 'ui-select-label' }),
          el('i', { class: 'bi bi-chevron-down ui-select-caret', 'aria-hidden': 'true' })
        ]);
        ...
        // 键盘可用性：保留原生 select 的上下键 / Home / End 行为
        trig.addEventListener('keydown', e => {
          const n = sel.options.length;
          if (!n) return;
          if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
            e.preventDefault();
            pick(sel, Math.max(0, Math.min(n - 1, sel.selectedIndex + (e.key === 'ArrowDown' ? 1 : -1))));
          } else if (e.key === 'Home') {
            e.preventDefault(); pick(sel, 0);
          } else if (e.key === 'End') {
            e.preventDefault(); pick(sel, n - 1);
          }
        });
        // 原生 change（含代码里手动派发的）都要同步显示
    ...
    return { enhance, refresh, refreshAll, labelOf };
  })();

  global.UI = {
    $, $$, el, clear, debounce, throttle, escapeHtml, copyText, formatBytes,
    fileToDataUrl, compressImage, Toast, Popover, Confirm, Theme, Lightbox, Select,
