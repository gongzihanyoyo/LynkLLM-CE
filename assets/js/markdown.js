...
(function (global) {
   ...
   * @param {object} [opts] { light: true } 流式输出期间使用：只挂交互按钮，
   *   跳过高亮 / 公式 / 图表等重活，避免高频重渲染阻塞主线程
  ...
  function enhance(root, opts) {
    ...
    // 流式期间到此为止；高亮 / 公式 / 图表等输出结束后再统一处理
    if (opts.light) return Promise.resolve();
