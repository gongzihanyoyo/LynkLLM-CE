...
(function (global) {
  ...
  const TAB_ORDER = ['general', 'personalize', 'models', 'tools', 'about'];
  ...
  const TOOL_TABS = ['tavily'];
  let curTool = 'tavily';

  /**
   * 顶层 tab 名的兼容映射。
   * 第 12 轮把 Tavily 从顶层 tab 移进了「工具」的子标签，
   * 但 Settings.open('tavily') 这个入口还在被别处调用（例如聊天区点
   * 「联网搜索」chip），所以老的 id 要能继续用 —— 统一归一到 tools。
   */
  ...
  function switchTab(name, opts) {
    const root = $('#settingsBackdrop');
    // 老的 'tavily' 入口归一成 'tools'（并选中 tavily 子标签）
    if (name === 'tavily') curTool = 'tavily';
  ...
  function buildBackup() {
    ...
    return Promise.all(jobs).then(res => {
      ...
      return packAssets(images, 'assets/images/', 'png', data.assets.images, files)
        ...
        .then(() => {
          const bg = global.Personalize ? Personalize.getBackground() : null;
          if (bg) {
  ...
  function exportData() {
    ...
    setTimeout(() => {
      buildBackup().then(blob => {
        ...
        const a = el('a', { href: url, download: 'lynkllm-ce-backup-' + stamp() + '.zip' });
