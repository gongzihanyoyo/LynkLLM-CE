...
(function (global) {
  ...
  function renderDataStats() {
    ...
    // 生成图片单独存在 IndexedDB，体积异步统计
    const imgCard = mk('—', T('statImages'));
    box.appendChild(imgCard);
    if (global.ImageStore) {
      ImageStore.stats().then(st => {
        if (!imgCard.parentNode) return;
        imgCard.firstChild.textContent = String(st.count);
        imgCard.lastChild.textContent = T('statImages') + (st.bytes ? ' · ' + UI.formatBytes(st.bytes) : '');
        imgCard.title = st.available ? '' : 'IndexedDB unavailable';
      }).catch(() => {});
    }
  ...
  function renderTavilyParams() {
    ...
    params.forEach(([name, type]) => {
      box.appendChild(el('span', { class: 'tp-chip' }, [
        el('span', { class: 'tp-name', text: name }),
        el('span', { class: 'tp-type', text: type })
      ]));
    });
  }
  ...
  function renderKindTabs() {
    ...
    const counts = { all: all.length, chat: 0, image: 0 };
  ...
  /** 关键词命中：名称 / 模型 ID / Base URL */
  function matchModel(m, q) {
    if (!q) return true;
    const hay = [m.name, m.model, m.baseUrl, m.kind === 'image' ? 'image 图片' : 'chat 对话']
  ...
  function renderModelList() {
    ...
    if (!models.length) {
      const icon = all.length && q ? 'bi-search'
        : (kind === 'image' ? 'bi-image' : (kind === 'chat' ? 'bi-chat-dots' : 'bi-hdd-stack'));
    ...
    models.forEach(m => {
      ...
      if (m.kind === 'image') {
        titleRow.appendChild(el('span', { class: 'model-tag kind' }, [
          el('i', { class: 'bi bi-image' }), document.createTextNode(T('kindImage'))
        ]));
      }
  ...
  function openModelEditor(model) {
    ...
    // 新建时沿用当前子分类作为默认类型
    const kind = m ? (m.kind || 'chat') : (Settings.kindFilter === 'image' ? 'image' : 'chat');
  ...
  function currentFormKind() {
    ...
    return (seg && seg.dataset.value === 'image') ? 'image' : 'chat';
  }

  /**
   * 依据模型类型调整表单：
   * 图片模型隐藏「上下文长度 / 思考强度 / 系统提示词 / 流式输出」，
   * 仅保留图片输入（图生图）能力开关。
   */
  function applyKindUI(kind) {
    const isImage = kind === 'image';
    const show = (sel, on) => { const n = $(sel); if (n) n.hidden = !on; };
    show('#rowContext', !isImage);
    show('#rowThinking', !isImage);
    show('#rowSystem', !isImage);
    show('#rowStream', !isImage);
    show('#rowTools', !isImage);
    ...
    if (kindHint) kindHint.textContent = T(isImage ? 'kindImageDesc' : 'kindChatDesc');

    const baseHint = $('#mfBaseUrlHint');
    if (baseHint) baseHint.textContent = T('fBaseUrlHint');
  }
  ...
  function readModelForm() {
    return {
      ...
      supportsTools: $('#mfTools') ? $('#mfTools').checked : false
    };
  }
  ...
  function clearAll() {
    Confirm.ask({
      ...
      if (!r.confirmed) return;
      Store.clearAll();
      // 生成图片存在 IndexedDB，需要单独清空
      const done = () => {
        Toast.success(T('allCleared'));
        setTimeout(() => global.location.reload(), 500);
      };
      if (global.ImageStore) ImageStore.clear().then(done).catch(done);
      else done();
    });
  }
