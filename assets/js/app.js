...
(function (global) {
  ...
  /* ======================================================================
     键盘快捷键
     ----------------------------------------------------------------------
     ⚠️ 这张表是**唯一数据源**：既驱动实际按键分派，又驱动「设置 → 快捷键」
     那一页的表格。所以不存在「文档写了但没实现 / 实现了但没写」的漂移 ——
     要加一个快捷键，只在这里加一行。
     修饰键统一写成 mod（Mac 上是 Cmd，其它平台是 Ctrl）。
     ====================================================================== */
  const SHORTCUTS = [
    {
      id: 'newChat',
    ...
    },
    {
      ...
      keys: ['mod', 'S'],
      labelKey: 'scExportChat',
      groupKey: 'scGroupChat',
      run: () => {
        if (global.Settings && Settings.exportConversation) Settings.exportConversation();
      }
    },
    {
      id: 'toggleSidebar',
      ...
      keys: ['mod', 'B'],
      labelKey: 'scToggleSidebar',
      groupKey: 'scGroupView',
      run: () => { if (isMobile()) toggleSidebar(); else toggleSidebar(); }
    },
    {
      id: 'openSettings',
      combo: { mod: true, key: '/' },
      keys: ['mod', '/'],
      labelKey: 'scOpenSettings',
      groupKey: 'scGroupView',
      run: () => {
        Settings.open('general');
        /* ⚠️ 必须写成 App.closeSidebarOnMobile()：它是后面才挂在 App 上的
           （`App.xxx = function(){}`），裸写函数名会直接 ReferenceError。 */
        App.closeSidebarOnMobile();
      }
    },
    /* 历史键位：Ctrl+Shift+O 新建对话。用户没要求删，保留以免打乱肌肉记忆。 */
    {
      id: 'newChatAlt',
      ...
      labelKey: 'scNewChat',
      groupKey: 'scGroupChat',
      hidden: true,          // 不在设置页重复列出（与 Ctrl+K 同功能）
      run: () => {
        Chat.startNewChat();
        const ta = $('#input');
        if (ta) ta.focus();
    ...
    },
    {
      ...
      groupKey: 'scGroupView',
      run: () => {
        if (Popover.isOpen()) { Popover.close(); return true; }
        if (App.generating) { Chat.stopGeneration(); return true; }
        return false;
      }
    }
  ];
  ...
  function shortcutList() {
    return SHORTCUTS.filter(s => !s.hidden).map(s => ({
      id: s.id,
      keys: s.keys.slice(),
      labelKey: s.labelKey,
      groupKey: s.groupKey
  ...
  /** 把组合键渲染成给人看的字符串（Mac 上显示 ⌘ 之类） */
  function shortcutKeysText(keys) {
  ...
  /** 把 e 的按键归一化成组合键键名（空格、斜杠等要转成稳定名字） */
  function keyNameOf(e) {
    const k = e.key;
    if (k === ' ') return 'Space';
    if (k === 'Escape') return 'Escape';
    if (k.length === 1) return k.toLowerCase();
    return k;   // 方向键、F1..F12 等保持原样
  }

  /** 事件是否命中某个 combo */
  function comboMatches(combo, e, name) {
    const mod = e.ctrlKey || e.metaKey;
    if (!!combo.mod !== !!mod) return false;
    if (!!combo.shift !== !!e.shiftKey) return false;
    if (!!combo.alt !== !!e.altKey) return false;
    return combo.key === name;
  }
  ...
  function isTypingTarget(t) {
    ...
    if (tag === 'input') {
      ...
      return ['button', 'checkbox', 'radio', 'submit', 'range', 'file'].indexOf(type) < 0;
    }
  ...
  function handleShortcut(e) {
    // 输入法组合中不处理（否则中文输入会误触发）
    if (e.isComposing || e.keyCode === 229) return;

    const name = keyNameOf(e);
    for (let i = 0; i < SHORTCUTS.length; i++) {
      const sc = SHORTCUTS[i];
      if (!comboMatches(sc.combo, e, name)) continue;

      /* Esc 比较特殊：没东西可关时不该 preventDefault，也不该被算作「已处理」，
         否则一些原生行为（比如退出全屏）会被莫名吞掉。 */
      if (sc.id === 'escape' && !Popover.isOpen() && !App.generating) return;

      // Esc 之外都带修饰键，因此在输入框里也应生效（Ctrl+S 保存、Ctrl+F 搜索都要能用）
      e.preventDefault();
      try { sc.run(); } catch (err) { console.error('[shortcut]', sc.id, err); }
      return;
    }
  }

  /** Ctrl+F：打开侧栏并聚焦搜索框（历史对话搜索） */
  ...
  /* ======================================================================
     ...
     ====================================================================== */
  function init() {
    // 把页面里所有原生 <select> 升级为自建下拉（展开列表无法适配毛玻璃，见 ui.js）
    if (UI.Select) UI.Select.enhance();
    Store.init();
    ...
    bindStorageSync();
    Settings.bind();
    // 与油猴增强脚本握手（没装脚本时这一步是纯空转，不影响任何功能）
    if (global.Enhancer) Enhancer.connect();
    ConvList.bind();
    bindComposer();
    bindSidebar();
    UI.Lightbox.bind();
  ...
  function bindGlobal() {
    ...
    /* 键盘快捷键统一走 handleShortcut（分派表见文件上方的 SHORTCUTS）。
       ⚠️ 第 13 轮改了键位：Ctrl+K 现在是「新建对话」（原来是聚焦搜索），
       搜索挪到了 Ctrl+F —— 用户明确要求的键位，别再改回去。 */
  ...
  /**
   * 展开侧栏。
   * ⚠️ **遮罩只在窄屏（抽屉模式）下才铺**。桌面端 `.sidebar` 是一个常驻的
   * flex 列（见 styles.css 的 `@media (max-width: 860px)`），根本没有「被盖住」
   * 的东西；此时若仍显示 `.sidebar-backdrop`，它是一层 `position:fixed; inset:0`
   * 且带模糊的覆盖层 —— 后果是**整个界面被蒙上一层、且第一次点击被它吃掉**
   * （点哪儿都只是把遮罩关掉）。
   * 以前无论桌面还是窄屏都无条件铺遮罩，`Ctrl+K` 聚焦搜索时就会这样；
   * 第 13 轮加了 `Ctrl+F` 同样会走这里，频率更高，所以顺手修掉。
   */
  ...
  function bindComposer() {
    ...
    /* 「+」按钮：输入框最左边的统一入口（上传图片 / Tavily 开关 / MCP 开关）。
       原来的图片按钮与「联网搜索」chip 都已收进这个菜单。 */
    if (plusBtn) plusBtn.addEventListener('click', e => {
      e.stopPropagation();
      if (Popover.isOpen()) { Popover.close(); return; }
      Chat.openPlusMenu(plusBtn);
    });
    if (fileInput) {
    ...
    /* 注意：聊天区原来还有一个独立的「联网搜索」chip（#btnWebSearch），
       第 13 轮已按用户要求移除 —— 它现在是「+」菜单里的一个开关。
       相关的绑定与文案同步逻辑也一并删掉了，别再往回加。 */
  ...
  function syncComposerForModel() {
    ...
    /* 「+」按钮：永远可见（它是输入区唯一的入口）。
       当前模型不支持图片输入时，只把「上传图片」那一项灰掉 —— 菜单里还有
       Tavily / MCP 开关，不能因为一个模型不支持图片就把整个入口藏起来。 */
    const plus = $('#btnPlus');
    if (plus) plus.title = T('plusMenu');
    ...
    /* 联网搜索 / MCP 现在都在「+」菜单里，不再有独立的 chip 需要同步。
       它们的状态由 Chat.openPlusMenu() 在每次打开时按当前对话实时读取 —— 
       这样天然不会出现「chip 上写着开启、实际已经关掉」的错位。 */
  ...
  /* 快捷键：设置页用它渲染表格，测试也用它断言「文档与实现同源」 */
  App.shortcutList = shortcutList;
  App.shortcutKeysText = shortcutKeysText;
  App.handleShortcut = handleShortcut;
  ...
  App.onModelChanged = function () {
    ...
    if (Settings.openTab === 'tools' && Settings.renderMcpList) Settings.renderMcpList();
    // 模型集合变了 → 增强脚本的转发白名单也要跟着更新
    // （否则「连上后才给某个模型开绕过开关」会被脚本按旧白名单拒掉）
