...
(function (global) {
  ...
  function syncComposerForModel() {
    ...
    if (thinkChip) thinkChip.hidden = isImage || !levels.length;
    ...
    const searchChip = $('#btnWebSearch');
    const canSearch = !!model && !isImage && !!model.supportsTools && Chat.tavilyReady();
    if (searchChip) {
    ...
    // 输入框占位符随模型类型切换
    const ta = $('#input');
    if (ta) ta.setAttribute('placeholder', T(isImage ? 'imageInputPlaceholder' : 'inputPlaceholder'));
