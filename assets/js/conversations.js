...
(function (global) {
  ...
  function render() {
    ...
    const all = Store.getConversations().sort(compareConversations);
    ...
    // 分组：置顶区永远在最上面（不参与日期分桶）
    const pinned = list.filter(c => c.pinned);
    const rest = list.filter(c => !c.pinned);

    if (pinned.length) {
      box.appendChild(el('div', { class: 'conv-group-label is-pinned', text: T('pinnedSection') }));
      pinned.forEach(c => box.appendChild(buildItem(c)));
    }

    const groups = groupByDate(rest);
    groups.forEach(g => {
      if (g.items.length > 1 || groups.length > 1 || pinned.length) {
  ...
  /**
   * 会话排序：置顶的排在最前，置顶区内部按「最近置顶」倒序，其余按更新时间倒序。
   */
  ...
  function buildItem(conv) {
    ...
    item.appendChild(el('i', {
      class: 'conv-item-icon bi ' + (conv.pinned ? 'bi-pin-angle-fill' : 'bi-chat-square-text')
    }));
  ...
  /* ---------- 更多操作 ---------- */
  /** 置顶 / 取消置顶（写回 store 后重排列表） */
  function togglePin(conv) {
    const fresh = Store.getConversation(conv.id);
    if (!fresh) return;
    fresh.pinned = !fresh.pinned;
    // 置顶时间只在「本次置顶」时刷新；取消置顶清零，避免残留旧值影响重新置顶的顺序
    fresh.pinnedAt = fresh.pinned ? Date.now() : 0;
    Store.upsertConversation(fresh);
    render();
    if (global.App) App.onConversationChanged();
    Toast.success(fresh.pinned ? T('pinned') : T('unpinned'));
  }
  ...
  function openMorePopover(anchor, conv) {
    ...
    box.appendChild(mk(conv.pinned ? 'bi-pin-angle-fill' : 'bi-pin-angle',
      conv.pinned ? T('unpin') : T('pin'), '', () => togglePin(conv)));
  ...
  function deleteConversation(conv) {
    ...
    const doDelete = () => {
      // 一并清理该对话生成的图片 / 音频（存在 IndexedDB 中，不清理会一直占用空间）
      const imgIds = [];
      const audioIds = [];
      (conv.messages || []).forEach(m => {
        if (Array.isArray(m.images)) m.images.forEach(i => { if (i && i.id) imgIds.push(i.id); });
        if (Array.isArray(m.audios)) m.audios.forEach(a => { if (a && a.id) audioIds.push(a.id); });
      });
      if (global.ImageStore && imgIds.length) ImageStore.removeMany(imgIds);
      if (global.AudioStore && audioIds.length) AudioStore.removeMany(audioIds);
  ...
  List.compareConversations = compareConversations;
  List.togglePin = togglePin;
