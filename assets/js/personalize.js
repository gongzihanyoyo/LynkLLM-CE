...
(function (global) {
  ...
  let bgUrl = null;
  /** 当前生效的效果开关与强度 */
  const cur = { fx: true, fxStrength: DEFAULT_STRENGTH, bgBlur: 0 };
   ...
   */
  /**
   * 背景图模糊强度 0-100 → 0-24px。
   * 0 表示「不模糊」——此时 data-bg-blur 关掉、body 自己画清晰的图。
   */
  function bgBlurPx(strength) {
    const n = Number(strength);
    const v = isNaN(n) ? 0 : Math.min(100, Math.max(0, n));
    return Math.round(v * 0.24);
  }

  /**
   * 应用「背景模糊」。
   * 注意这是**背景图自身**的模糊（filter 作用在承载图片的伪元素上），
   * 与面板的毛玻璃（backdrop-filter）是两件正交的事，互不影响。
   */
  function applyBgBlur(strength) {
    const root = doc.documentElement;
    const px = bgBlurPx(strength);
    if (px > 0 && bgCache) {
      root.style.setProperty('--bg-blur', px + 'px');
      root.setAttribute('data-bg-blur', 'on');
    } else {
   ...
   */
  function applyBgBlurSetting(strength) {
    cur.bgBlur = Number(strength) || 0;
    applyBgBlur(cur.bgBlur);
  }
  ...
  function applyBackground() {
    ...
    if (!bgCache) {
      ...
      // ⚠️ 这里必须也重算一次背景模糊：否则「移除背景图」会留下
      // data-bg-blur="on" 与 --bg-blur 的残留状态（模糊层已无作用对象）
      applyBgBlur(0);
      syncAttrs();
      return;
    }
    ...
    applyBgBlur(cur.bgBlur);      // 图片换了/被移除，模糊层跟着重算
    syncAttrs();
  }
  ...
  global.Personalize = {
    ...
    applyAccent,
    applyFx,
    applyBgBlur: applyBgBlurSetting,
    bgBlurPx,
    fxValues,
    loadBackground,
    setBackground,
