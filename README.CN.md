...
- 直接连接接口，无需中转，高速输出。
- 可自由管理多个模型，支持图片输入与输出。
- 支持接入 [Tavily](https://www.tavily.com)，让 AI 自动搜索信息。
- 提供上下文长度警告，防止因长度过长导致意外中断。
- 提供 [Tampermonkey 脚本](https://www.tampermonkey.net/script_installation.php#url=https://lynkllm-ce.pages.dev/LynkLLM-CE-Enhancer.user.js)，以绕过 CORS 限制。
...
*\*此在线演示站点上的版本可能比 GitHub 上的代码更新。*

## 注意事项

- 对于某些带有 CORS 限制的 API，请[安装增强脚本](https://www.tampermonkey.net/script_installation.php#url=https://lynkllm-ce.pages.dev/LynkLLM-CE-Enhancer.user.js)。
- 目前仅支持 OpenAI 格式；其他格式暂不可用。
...
- [智谱 BigModel](https://www.bigmodel.cn)
- OrcaRouter
